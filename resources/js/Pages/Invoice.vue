        ...
        <div class="mb-2 pb-2 border-b text-center">
          <div v-if="invoice.payment?.status == 'paid'">
            <div class="flex items-center justify-center">
              <svg
                class="w-20 h-20 text-green-400 animate-pulse"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
          ...
          <div v-else-if="invoice.payment?.status == 'timeout'">
            ...
            <p class="font-bold">Payment expired / timeout</p>
          </div>

          <div v-else>
            <div class="flex items-center justify-center">
              <svg
                class="w-20 h-20 text-yellow-400 animate-bounce"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
                xmlns="http://www.w3.org/2000/svg"
            ...
            <p>Check your notification or open OVO app to complete the payment</p>
          </div>
        </div>
        <div class="mb-2">
...
export default {
  ...
  setup(props) {
    ...
    onMounted(() => {
      setTimeout(() => {
        if (
          props.invoice.payment === undefined ||
          props.invoice.payment?.status == "unpaid"
        ) {
          Inertia.get(route("invoice.check", props.invoice.id));
        }
      }, 3000);
